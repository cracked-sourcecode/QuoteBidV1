...
async function processPricingTick(): Promise<void> {
  ...
  try {
    ...
    for (const opp of liveOpps) {
      ...
      const isInCooldown = !canUpdate(opp.last_price_update, cooldownMinutes);
      ...
      // CRITICAL FIX: Always allow ambient decay, even during cooldown
      if (newPrice !== currentPrice) {
        const isPriceDecrease = priceDelta < 0;
        const isAmbientDecay = pricingResult.meta.driftApplied || Math.abs(priceDelta) <= pricingConfig.priceStep;
        
        // Allow immediate updates for:
        // 1. Not in cooldown (normal operation)
        ...
        console.log(`⏳  OPP ${opp.id} – no price change needed (in cooldown)`);
      }
    }
